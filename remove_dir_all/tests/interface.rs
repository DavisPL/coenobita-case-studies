...
fn ensure_empty_dir_missing_dir() {
    ...
    remove_dir_all::ensure_empty_dir(&path, |s| Path::new(s)).unwrap();
    assert_empty(&path);
}
...
fn ensure_empty_dir_is_file() {
    ...
    remove_dir_all::ensure_empty_dir(&path, |s| Path::new(s)).unwrap_err();
    assert_exists(&path);
}
...
fn removes_empty() {
    ...
    remove_dir_all::remove_dir_all(&path, |s| Path::new(s)).unwrap();
...
fn removes_symlinks() {
    ...
    remove_dir_all::ensure_empty_dir(&root, |s| Path::new(s)).unwrap();
    assert_exists(&root);
