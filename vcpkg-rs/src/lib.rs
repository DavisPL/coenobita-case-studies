...
//! ```rust,no_run
//! // build.rs
//! vcpkg::find_package("libssh2", |p1, p2| p1.join(Path::new(p2))).unwrap();
//! ```
//!
...
//! vcpkg::Config::new()
//!     .emit_includes(true)
//!     .find_package("zlib", |p1, p2| p1.join(Path::new(p2))).unwrap();
//! ```
//!
...
type Join = fn(&Path, &OsStr) -> PathBuf;
type Generate = fn(&OsStr) -> &Path;
type Push = fn(&mut PathBuf, &OsStr);
type Pop = fn(&mut PathBuf) -> bool;
...
#[doc(hidden)]
pub fn probe_package(name: &str, join: Join, generate: Generate, push: Push, pop: Pop) -> Result<Library, Error> {
    Config::new().probe(name, join, generate, push, pop)
}
...
pub fn find_package(package: &str, join: Join, generate: Generate, push: Push, pop: Pop) -> Result<Library, Error> {
    Config::new().find_package(package, join, generate, push, pop)
}

/// Find the vcpkg root
#[doc(hidden)]
pub fn find_vcpkg_root(cfg: &Config, generate: Generate, join: Join, push: Push, pop: Pop) -> Result<PathBuf, Error> {
    ...
    if let Ok(ref local_app_data) = env::var("LOCALAPPDATA") {
        let vcpkg_user_targets_path = join(
            &join(generate(local_app_data.as_ref()),"vcpkg".as_ref()),
            "vcpkg.user.targets".as_ref());
    ...
    if let Some(path) = env::var_os("OUT_DIR") {
        ...
        while pop(&mut path) {
            ...
            push(&mut try_root, "vcpkg".as_ref());
            push(&mut try_root, ".vcpkg-root".as_ref());
            if try_root.exists() {
                pop(&mut try_root);
                ...
                push(&mut cv_cfg, "downloads".as_ref());
                push(&mut cv_cfg, "cargo-vcpkg.toml".as_ref());
                if cv_cfg.exists() {
                    return Ok(try_root);
                }
            }
...
fn validate_vcpkg_root(path: &PathBuf, push: Push) -> Result<(), Error> {
    let mut vcpkg_root_path = path.clone();
    push(&mut vcpkg_root_path, ".vcpkg-root".as_ref());
...
fn find_vcpkg_target(cfg: &Config, target_triplet: &TargetTriplet, join: Join, generate: Generate, push: Push, pop: Pop) -> Result<VcpkgTarget, Error> {
    let vcpkg_root = try!(find_vcpkg_root(&cfg, generate, join, push, pop));
    try!(validate_vcpkg_root(&vcpkg_root, push));

    let mut base = cfg
        .vcpkg_installed_root
        .clone()
        .or(env::var_os("VCPKG_INSTALLED_ROOT").map(PathBuf::from))
        .unwrap_or(join(&vcpkg_root, "installed".as_ref()));

    let status_path = join(&base, "vcpkg".as_ref());

    push(&mut base, &target_triplet.triplet.as_ref());

    let lib_path = join(&base, "lib".as_ref());
    let bin_path = join(&base, "bin".as_ref());
    let include_path = join(&base, "include".as_ref());
    let packages_path = join(&vcpkg_root, "packages".as_ref());
...
fn load_port_manifest(
    ...
    let manifest_file = join(&join(path, "info".as_ref()), &format!(
        "{}_{}_{}.list",
    ...
    let dll_prefix = join(generate(&vcpkg_target.target_triplet.triplet.as_ref()), "bin".as_ref());
    let lib_prefix = join(generate(&vcpkg_target.target_triplet.triplet.as_ref()), "lib".as_ref());

    for line in file.lines() {
        let line = line.unwrap();

        let file_path = generate(&line.as_ref());
    ...
    let pkg_config_prefix = join(
        &join(&join(&vcpkg_target.packages_path,
        &format!("{}_{}", port, vcpkg_target.target_triplet.triplet).as_ref()),
        "lib".as_ref()),
        "pkgconfig".as_ref());
...
fn load_ports(target: &VcpkgTarget, join: Join, generate: Generate) -> Result<BTreeMap<String, Port>, Error> {
    ...
    let status_filename = join(&target.status_path, "status".as_ref());
    load_port_file(&status_filename, &mut port_info).ok();

    // load updates to the status file that have yet to be normalized
    let status_update_dir = join(&target.status_path, "updates".as_ref());
...
impl Config {
    ...
    /// set on the builder.
    pub fn find_package(&mut self, port_name: &str, join: Join, generate: Generate, push: Push, pop: Pop) -> Result<Library, Error> {
        ...
        let vcpkg_target = try!(find_vcpkg_target(&self, &msvc_target, join, generate, push, pop));
        ...
        if self.required_libs.is_empty() {
            let ports = try!(load_ports(&vcpkg_target, join, generate));
        ...
        try!(self.emit_libs(&mut lib, &vcpkg_target, push));

        if self.copy_dlls {
            try!(self.do_dll_copy(&mut lib, generate, push));
        }
    ...
    /// Deprecated in favor of the find_package function
    #[doc(hidden)]
    pub fn probe(&mut self, port_name: &str, join: Join, generate: Generate, push: Push, pop: Pop) -> Result<Library, Error> {
        // determine the target type, bailing out if it is not some
        // kind of msvc
        let msvc_target = try!(self.get_target_triplet());
    ...
    fn emit_libs(&mut self, lib: &mut Library, vcpkg_target: &VcpkgTarget, push: Push) -> Result<(), Error> {
        for required_lib in &self.required_libs {
            ...
            push(&mut lib_location, &(required_lib.clone() + "." + &vcpkg_target.target_triplet.lib_suffix).as_ref());
        ...
        if !vcpkg_target.target_triplet.is_static {
            for required_dll in &self.required_dlls {
                ...
                push(&mut dll_location, &(required_dll.clone() + ".dll").as_ref());
    ...
    fn do_dll_copy(&mut self, lib: &mut Library, generate: Generate, push: Push) -> Result<(), Error> {
        if let Some(target_dir) = env::var_os("OUT_DIR") {
            if !lib.found_dlls.is_empty() {
                for file in &lib.found_dlls {
                    let mut dest_path = generate(target_dir.as_os_str()).to_path_buf();
                    push(&mut dest_path, file.file_name().unwrap());
                    try!(
...
mod tests {
    ...
    fn do_nothing_for_unsupported_target() {
        ...
        assert!(match ::probe_package("foo", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()) {
    ...
    //     env::set_var("TARGET", "x86_64-pc-windows-msvc");
    //     println!("Result is {:?}", ::find_package("libmysql", |p1, p2| p1.join(Path::new(p2))));
    //     assert!(match ::find_package("libmysql", |p1, p2| p1.join(Path::new(p2))) {
    ...
    fn static_build_finds_lib() {
        ...
        env::set_var("TARGET", "x86_64-pc-windows-msvc");
        let tmp_dir = tempdir(join).unwrap();
        ...
        println!("Result is {:?}", ::find_package("libmysql", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()));
        assert!(match ::find_package("libmysql", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()) {
            Ok(_) => true,
            _ => false,
        });
        clean_env();
    ...
    fn handle_multiline_description() {
        ...
        println!("Result is {:?}", ::find_package("graphite2", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()));
        assert!(match ::find_package("graphite2", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()) {
            Ok(_) => true,
            _ => false,
        });
        clean_env();
    ...
    fn link_libs_required_by_optional_features() {
        ...
        println!("Result is {:?}", ::find_package("harfbuzz", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()));
        assert!(match ::find_package("harfbuzz", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()) {
            Ok(lib) => lib
                .cargo_metadata
                .iter()
    ...
    fn link_dependencies_after_port() {
        ...
        let lib = ::find_package("harfbuzz", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()).unwrap();
    ...
    fn custom_target_triplet_in_config() {
        ...
        let harfbuzz = ::Config::new()
            ...
            .find_package("harfbuzz", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop());
    ...
    fn custom_target_triplet_by_env_no_default() {
        ...
        let harfbuzz = ::find_package("harfbuzz", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop());
        println!("Result with inference is {:?}", &harfbuzz);
        assert!(harfbuzz.is_err());

        env::set_var("VCPKGRS_TRIPLET", "x64-osx");
        let harfbuzz = ::find_package("harfbuzz", |p1, p2| p1.join(Path::new(p2)), |s| Path::new(s), |p1, p2| p1.push(Path::new(p2)), |p| p.pop()).unwrap();
    ...
    fn vcpkg_test_tree_loc(name: &str) -> PathBuf {
        ...
        path.push(Path::new(&env::var("CARGO_MANIFEST_DIR").unwrap()));
        path.push(Path::new("test-data"));
