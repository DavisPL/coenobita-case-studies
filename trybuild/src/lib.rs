...
#![doc(html_root_url = "https://docs.rs/trybuild/1.0.101")]
...
use run::WithExtension;

pub(crate) type Collect = fn(Peekable<Components>) -> PathBuf;
pub(crate) type Generate = fn(&OsStr) -> &Path;
pub(crate) type Join = fn(&OsStr, &OsStr) -> PathBuf;
...
impl TestCases {
    ...
    pub fn new(with_extension: WithExtension, collect: Collect, join: Join, generate: Generate) -> Self {
        TestCases {
...
impl Drop for TestCases {
    fn drop(&mut self) {
        if !thread::panicking() {
            self.runner.borrow_mut().run(self.with_extension, self.collect, self.join, self.generate);
        }
    }
}
