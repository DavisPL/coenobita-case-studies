...
pub(crate) type WithExtension = fn(&Path, &OsStr) -> PathBuf;
...
impl Runner {
    pub(crate) fn run(&mut self, with_extension: WithExtension, collect: Collect, join: Join, generate: Generate) {
        let mut tests = expand_globs(&self.tests, collect, join, generate);
        filter(&mut tests);

        let (project, _lock) = (|| {
            let mut project = self.prepare(&tests, join, generate)?;
        ...
        if tests.is_empty() {
            ...
            report = match self.run_all(&project, tests, with_extension, join, generate) {
            ...
            for test in tests {
                match test.run(&project, with_extension, join, generate) {
                    Ok(Outcome::Passed) => {}
    ...
    fn prepare(&self, tests: &[ExpandedTest], join: Join, generate: Generate) -> Result<Project> {
        let Metadata {
        ...
        let source_dir = cargo::manifest_dir(join)?;
        let source_manifest = dependencies::get_manifest(&source_dir, join)?;

        let mut features = features::find(join);
        ...
        fs::create_dir_all(&project_dir.as_ref())?;
    ...
    fn make_manifest(
        ...
        let crate_name = source_manifest.package.name;
        let workspace_manifest = dependencies::get_workspace_manifest(workspace, join);
        ...
        let cargo_toml_path = join(source_dir.as_ref().as_os_str(), "Cargo.toml".as_ref());
        ...
        manifest.bins.push(Bin {
            ...
            path: generate("main.rs".as_ref()).to_owned(),
        });
        ...
        for expanded in tests {
            if expanded.error.is_none() {
                manifest.bins.push(Bin {
                    ...
                    path: source_dir.join(&expanded.test.path, join),
                });
            }
        }
    ...
    fn run_all(&self, project: &Project, tests: Vec<ExpandedTest>, with_extension: WithExtension, join: Join, generate: Generate) -> Result<Report> {
        let mut report = Report {
            failures: 0,
            created_wip: 0,
        };
        ...
        for t in &tests {
            let src_path = project.source_dir.join(&t.test.path, join);
        ...
        for mut t in tests {
            ...
            if t.error.is_none() {
                ...
                match t.test.check(project, &t.name, this_test, "", with_extension, generate, join) {
                    Ok(Outcome::Passed) => {}
                    Ok(Outcome::CreatedWip) => report.created_wip += 1,
...
impl Test {
    fn run(&self, project: &Project, name: &Name, with_extension: WithExtension, join: Join, generate: Generate) -> Result<Outcome> {
        let show_expected = project.has_pass && project.has_compile_fail;
        message::begin_test(self, show_expected);
        check_exists(&self.path)?;

        let mut path_map = Map::new();
        let src_path = project.source_dir.join(&self.path, join);
        ...
        let fallback = Stderr::default();
        let this_test = parsed.stderrs.get(&src_path).unwrap_or(&fallback);
        self.check(project, name, this_test, &parsed.stdout, with_extension, generate, join)
    }
    ...
    fn check_compile_fail(
        ...
        let stderr_path = with_extension(&self.path, "stderr".as_ref());

        if !stderr_path.exists() {
            let outcome = match project.update {
                Update::Wip => {
                    let wip_dir = generate("wip".as_ref());
                    fs::create_dir_all(wip_dir)?;
                    let gitignore_path = join(wip_dir.as_os_str(), ".gitignore".as_ref());
                    fs::write(gitignore_path, "*\n")?;
                    let stderr_name = stderr_path
                        .file_name()
                        .unwrap_or_else(|| OsStr::new("test.stderr"));
                    let wip_path = join(wip_dir.as_os_str(), stderr_name.as_ref());
...
impl ExpandedTest {
    fn run(self, project: &Project, with_extension: WithExtension, join: Join, generate: Generate) -> Result<Outcome> {
        match self.error {
            None => self.test.run(project, &self.name, with_extension, join, generate),
            Some(error) => {
                let show_expected = false;
