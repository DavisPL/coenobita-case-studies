...
fn cargo(project: &Project) -> Command {
    ...
    cmd.current_dir(&project.dir.as_ref().as_os_str());
...
pub(crate) fn manifest_dir(join: Join) -> Result<Directory> {
    if let Some(manifest_dir) = env::var_os("CARGO_MANIFEST_DIR") {
        return Ok(Directory::from(manifest_dir));
    }
    let mut dir = Directory::current()?;
    loop {
        if join(dir.as_ref().as_os_str(), "Cargo.toml".as_ref()).exists() {
            return Ok(dir);
        }
