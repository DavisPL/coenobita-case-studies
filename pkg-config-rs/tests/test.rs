...
fn generate(s: &str) -> PathBuf {
    PathBuf::from(s)
}

fn join() -> impl Fn(&PathBuf, &PathBuf) -> PathBuf {
    |p1: &PathBuf, p2: &PathBuf| p1.join(p2)
}
...
fn reset() {
    ...
    env::remove_var("HOST");
    env::set_var("PKG_CONFIG_PATH", env::current_dir().unwrap().join(Path::new("tests")));
}

fn find(name: &str) -> Result<pkg_config::Library, Error> {
    pkg_config::probe_library(name, generate, |p1, p2| p1.join(p2))
}
...
fn atleast_version_ok() {
    ...
    pkg_config::Config::new()
        ...
        .probe("foo", generate, |p1, p2| p1.join(p2))
        .unwrap();
}
...
fn range_version_range_ok() {
    ...
    pkg_config::Config::new()
        ...
        .probe("escape", generate, |p1, p2| p1.join(p2))
        .unwrap();
}
