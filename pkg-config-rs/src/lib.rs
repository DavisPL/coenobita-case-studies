...
#[doc(hidden)]
pub fn find_library<G: Fn(&str) -> PathBuf, J: Fn(&PathBuf, &PathBuf) -> PathBuf>(name: &str, generator: G, join: J) -> Result<Library, String> {
    probe_library(name, generator, join).map_err(|e| e.to_string())
}

/// Simple shortcut for using all default options for finding a library.
pub fn probe_library<G: Fn(&str) -> PathBuf, J: Fn(&PathBuf, &PathBuf) -> PathBuf>(name: &str, generator: G, join: J) -> Result<Library, Error> {
    Config::new().probe(name, generator, join)
}
...
impl Config {
    ...
    #[doc(hidden)]
    pub fn find<G: Fn(&str) -> PathBuf, J: Fn(&PathBuf, &PathBuf) -> PathBuf>(&self, name: &str, generator: G, join: J) -> Result<Library, String> {
        self.probe(name, generator, join).map_err(|e| e.to_string())
    }
    ...
    /// `pkg-config` is run.
    pub fn probe<G: Fn(&str) -> PathBuf, J: Fn(&PathBuf, &PathBuf) -> PathBuf>(&self, name: &str, generator: G, join: J) -> Result<Library, Error> {
        ...
        library.parse_libs_cflags(name, &output, self, generator, join);
...
impl Library {
    ...
    fn parse_libs_cflags<G: Fn(&str) -> PathBuf, J: Fn(&PathBuf, &PathBuf) -> PathBuf>(&mut self, name: &str, output: &[u8], config: &Config, generator: G, join: J) {
            ...
            match flag {
                ...
                "-l" => {
                    ...
                    if val.starts_with(':') {
                        ...
                        config.print_metadata(&meta);
                    } else if statik && is_static_available(val, &system_roots, &dirs, &join) {
...
fn is_static_available<J: Fn(&PathBuf, &PathBuf) -> PathBuf>(name: &str, system_roots: &[PathBuf], dirs: &[PathBuf], join: J) -> bool {
    let libnames = {
    ...
    dirs.iter().any(|dir| {
        let library_exists = libnames.iter().any(|libname| join(dir, &libname.into()).exists());
    ...
    fn system_library_mac_test() {
        ...
        if Path::new("/usr/local/lib/libpng16.a").exists() {
            ...
            let libpng = Config::new()
                .range_version("1".."99")
                .probe("libpng16", |s| PathBuf::from(s), |p1, p2| p1.join(p2))
                .unwrap();
