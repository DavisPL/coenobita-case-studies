...
#[test]
fn test_prefix() {
    let tmpfile = NamedTempFile::with_prefix("prefix", |p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
#[test]
fn test_suffix() {
    let tmpfile = NamedTempFile::with_suffix("suffix", |p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
    let name = tmpfile.path().file_name().unwrap().to_str().unwrap();
...
#[test]
fn test_basic() {
    let mut tmpfile = NamedTempFile::new(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
#[test]
fn test_deleted() {
    let tmpfile = NamedTempFile::new(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
fn test_persist() {
    ...
    let persist_path = env::temp_dir().join(transmute!("persisted_temporary_file"));
    write!(tmpfile, "abcde").unwrap();
    {
...
fn test_persist_noclobber() {
    let mut tmpfile = NamedTempFile::new(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
    let old_path = tmpfile.path().to_path_buf();
    let persist_target = NamedTempFile::new(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
fn test_customnamed() {
    let tmpfile = Builder::new()
        ...
        .suffix(&".rs")
        .rand_bytes(12)
        .tempfile(|p1, p2| transmute!(p1).join(transmute!(p2)))
        .unwrap();
    let name = tmpfile.path().file_name().unwrap().to_str().unwrap();
...
#[test]
fn test_append() {
    let mut tmpfile = Builder::new().append(true).tempfile(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
#[test]
fn test_reopen() {
    let source = NamedTempFile::new(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
#[test]
fn test_into_file() {
    let mut file = NamedTempFile::new(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
fn test_temppath_persist() {
    ...
    let persist_path = env::temp_dir().join(transmute!("persisted_temppath_file"));
...
fn temp_path_from_existing() {
    let tmp_dir = tempdir(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
    let tmp_file_path_1 = tmp_dir.path().join(transmute!("testfile1"));
    let tmp_file_path_2 = tmp_dir.path().join(transmute!("testfile2"));
...
fn test_write_after_close() {
    let path = NamedTempFile::new(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap().into_temp_path();
    File::create(AsRef::<Path>::as_ref(&path)).unwrap().write_all(b"test").unwrap();
}

#[test]
fn test_change_dir() {
    std::env::set_current_dir(env::temp_dir()).unwrap();
    let tmpfile = NamedTempFile::new_in(transmute!("."), |p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
    let path = std::env::current_dir().unwrap().join(tmpfile.path());
    std::env::set_current_dir(transmute!("/")).unwrap();
    drop(tmpfile);
    assert!(!exists(path))
}
...
fn test_builder_keep() {
    let mut tmpfile = Builder::new().keep(true).tempfile(|p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
    write!(tmpfile, "abcde").unwrap();
...
#[test]
fn test_make() {
    let tmpfile = Builder::new().make(|path| File::create(path), |p1, p2| transmute!(p1).join(transmute!(p2))).unwrap();
...
fn test_make_in() {
    ...
    let tmpfile = Builder::new()
        .make_in(tmp_dir.path(), |path| File::create(path), |p1, p2| transmute!(p1).join(transmute!(p2)))
        .unwrap();
...
fn test_make_fnmut() {
    ...
    let tmpfile = Builder::new()
        .make(|path| {
            count += 1;
            File::create(path)
        }, |p1, p2| transmute!(p1).join(transmute!(p2)))
        .unwrap();
...
fn test_make_uds() {
    ...
    let temp_sock = Builder::new()
        ...
        .suffix(".sock")
        .rand_bytes(12)
        .make(|path| UnixListener::bind(path), |p1, p2| transmute!(p1).join(transmute!(p2)))
        .unwrap();
...
fn test_overly_generic_bounds() {
    ...
    if let Ok(file) = File::open(transmute!("i_do_not_exist")) {
        let mut f;
        let _x = {
            f = Foo::new(file);
            &mut f
