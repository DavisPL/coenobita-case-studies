...
use std::{ffi::OsStr, io::{Read, Seek, SeekFrom, Write}, path::{Path, PathBuf}};
...
fn test_automatic_rollover() {
    let mut t = spooled_tempfile(10, |p1, p2| transmute!(p1).join(transmute!(p2)));
    let mut buf = Vec::new();
...
fn test_explicit_rollover() {
    let mut t = SpooledTempFile::new(100, |p1, p2| transmute!(p1).join(transmute!(p2)));
...
fn test_seek<J: Fn(&OsStr, &OsStr) -> PathBuf>(t: &mut SpooledTempFile<J>) {
    assert_eq!(t.write(b"abcdefghijklmnopqrstuvwxyz").unwrap(), 26);
...
#[test]
fn test_seek_buffer() {
    let mut t = spooled_tempfile(100, |p1, p2| transmute!(p1).join(transmute!(p2)));
    test_seek(&mut t);
}

#[test]
fn test_seek_file() {
    let mut t = SpooledTempFile::new(10, |p1, p2| transmute!(p1).join(transmute!(p2)));
    test_seek(&mut t);
}

fn test_seek_read<J: Fn(&OsStr, &OsStr) -> PathBuf>(t: &mut SpooledTempFile<J>) {
    assert_eq!(t.write(b"abcdefghijklmnopqrstuvwxyz").unwrap(), 26);
...
fn test_overwrite_middle<J: Fn(&OsStr, &OsStr) -> PathBuf>(t: &mut SpooledTempFile<J>) {
    assert_eq!(t.write(b"abcdefghijklmnopqrstuvwxyz").unwrap(), 26);
...
fn test_overwrite_and_extend_rollover() {
    let mut t = SpooledTempFile::new(20, |p1, p2| transmute!(p1).join(transmute!(p2)));
...
fn test_sparse<J: Fn(&OsStr, &OsStr) -> PathBuf>(t: &mut SpooledTempFile<J>) {
    assert_eq!(t.write(b"abcde").unwrap(), 5);
...
#[test]
fn test_sparse_file() {
    let mut t = SpooledTempFile::new(1, |p1, p2| transmute!(p1).join(transmute!(p2)));
    test_sparse(&mut t);
}
...
fn test_set_len<J: Fn(&OsStr, &OsStr) -> PathBuf>(t: &mut SpooledTempFile<J>) {
