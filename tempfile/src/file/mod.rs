...
/// # Ok::<(), std::io::Error>(())
/// ```
pub fn tempfile<J: Fn(&OsStr, &OsStr) -> PathBuf>(join: J) -> io::Result<File> {
    tempfile_in(env::temp_dir(), join)
}
...
/// # Ok::<(), std::io::Error>(())
/// ```
pub fn tempfile_in<P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(dir: P, join: J) -> io::Result<File> {
    imp::create(dir.as_ref(), join)
}
...
impl NamedTempFile<File> {
    ...
    pub fn new<J: Fn(&OsStr, &OsStr) -> PathBuf>(join: J) -> io::Result<NamedTempFile> {
        Builder::new().tempfile(join)
    }
    ...
    ///
    /// [`NamedTempFile::new()`]: #method.new
    pub fn new_in<P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(dir: P, join: J) -> io::Result<NamedTempFile> {
        Builder::new().tempfile_in(dir, join)
    }
    ...
    ///
    /// [`NamedTempFile::new()`]: #method.new
    pub fn with_suffix<S: AsRef<OsStr>, J: Fn(&OsStr, &OsStr) -> PathBuf>(suffix: S, join: J) -> io::Result<NamedTempFile> {
        Builder::new().suffix(&suffix).tempfile(join)
    }
    /// Create a new named temporary file with the specified filename suffix,
    ...
    ///
    /// [`NamedTempFile::new()`]: #method.new
    pub fn with_suffix_in<S: AsRef<OsStr>, P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(
        suffix: S,
        dir: P,
        join: J
    ) -> io::Result<NamedTempFile> {
        Builder::new().suffix(&suffix).tempfile_in(dir, join)
    }
    ...
    ///
    /// [`NamedTempFile::new()`]: #method.new
    pub fn with_prefix<S: AsRef<OsStr>, J: Fn(&OsStr, &OsStr) -> PathBuf>(prefix: S, join: J) -> io::Result<NamedTempFile> {
        Builder::new().prefix(&prefix).tempfile(join)
    }
    /// Create a new named temporary file with the specified filename prefix,
    /// in the specified directory.
    ///
    ...
    ///
    /// [`NamedTempFile::new()`]: #method.new
    pub fn with_prefix_in<S: AsRef<OsStr>, P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(
        prefix: S,
        dir: P,
        join: J
    ) -> io::Result<NamedTempFile> {
        Builder::new().prefix(&prefix).tempfile_in(dir, join)
    }
}
...
pub(crate) fn create_named<J: Fn(&OsStr, &OsStr) -> PathBuf>(
    mut path: PathBuf,
    ...
    if !path.is_absolute() {
        path = join(std::env::current_dir()?.as_ref(), path.as_ref());
    }
