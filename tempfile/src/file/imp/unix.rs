...
fn create_unlinked<J: Fn(&OsStr, &OsStr) -> PathBuf>(path: &Path, join: J) -> io::Result<File> {
    let tmp;
    // shadow this to decrease the lifetime. It can't live longer than `tmp`.
    let mut path = path;
    if !path.is_absolute() {
        let cur_dir = std::env::current_dir()?;
        tmp = join(cur_dir.as_ref(), path.as_ref());
        path = &tmp;
    }
...
pub fn create<J: Fn(&OsStr, &OsStr) -> PathBuf>(dir: &Path, join: J) -> io::Result<File> {
    create_unix(dir, join)
}

fn create_unix<J: Fn(&OsStr, &OsStr) -> PathBuf>(dir: &Path, join: J) -> io::Result<File> {
    util::create_helper(
        dir,
        OsStr::new(".tmp"),
        OsStr::new(""),
        crate::NUM_RAND_CHARS,
        |path| create_unlinked(&path, &join),
        &join
    )
}
