...
pub fn tempdir<J: Fn(&OsStr, &OsStr) -> PathBuf>(join: J) -> io::Result<TempDir> {
    TempDir::new(join)
}
...
/// [`TempDir`]: struct.TempDir.html
/// [resource-leaking]: struct.TempDir.html#resource-leaking
pub fn tempdir_in<P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(dir: P, join: J) -> io::Result<TempDir> {
    TempDir::new_in(dir, join)
}
...
impl TempDir {
    ...
    ///
    /// [`Builder`]: struct.Builder.html
    pub fn new<J: Fn(&OsStr, &OsStr) -> PathBuf>(join: J) -> io::Result<TempDir> {
        Builder::new().tempdir(join)
    }
    ...
    /// # Ok::<(), std::io::Error>(())
    /// ```
    pub fn new_in<P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(dir: P, join: J) -> io::Result<TempDir> {
        Builder::new().tempdir_in(dir, join)
    }
    ...
    /// # Ok::<(), std::io::Error>(())
    /// ```
    pub fn with_prefix<S: AsRef<OsStr>, J: Fn(&OsStr, &OsStr) -> PathBuf>(prefix: S, join: J) -> io::Result<TempDir> {
        Builder::new().prefix(&prefix).tempdir(join)
    }
    ...
    /// # Ok::<(), std::io::Error>(())
    /// ```
    pub fn with_suffix<S: AsRef<OsStr>, J: Fn(&OsStr, &OsStr) -> PathBuf>(suffix: S, join: J) -> io::Result<TempDir> {
        Builder::new().suffix(&suffix).tempdir(join)
    }
    ...
    pub fn with_suffix_in<S: AsRef<OsStr>, P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(
        ...
        Builder::new().suffix(&suffix).tempdir_in(dir, join)
    }
    ...
    pub fn with_prefix_in<S: AsRef<OsStr>, P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(
        ...
        Builder::new().prefix(&prefix).tempdir_in(dir, join)
    }
