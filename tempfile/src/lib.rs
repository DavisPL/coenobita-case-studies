...
impl<'a, 'b> Builder<'a, 'b> {
    ...
    pub fn tempfile<J: Fn(&OsStr, &OsStr) -> PathBuf>(&self, join: J) -> io::Result<NamedTempFile> {
        self.tempfile_in(env::temp_dir(), join)
    }
    ...
    /// [security]: struct.NamedTempFile.html#security
    /// [resource-leaking]: struct.NamedTempFile.html#resource-leaking
    pub fn tempfile_in<P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(&self, dir: P, join: J) -> io::Result<NamedTempFile> {
        util::create_helper(
            dir.as_ref(),
            self.prefix,
            self.suffix,
    ...
    pub fn tempdir<J: Fn(&OsStr, &OsStr) -> PathBuf>(&self, join: J) -> io::Result<TempDir> {
        self.tempdir_in(env::temp_dir(), join)
    }
    ...
    ///
    /// [resource-leaking]: struct.TempDir.html#resource-leaking
    pub fn tempdir_in<P: AsRef<Path>, J: Fn(&OsStr, &OsStr) -> PathBuf>(&self, dir: P, join: J) -> io::Result<TempDir> {
        let storage;
        let mut dir = dir.as_ref();
        if !dir.is_absolute() {
            let cur_dir = std::env::current_dir()?;
            storage = join(cur_dir.as_ref(), dir.as_ref());
            dir = &storage;
        }
    ...
    /// [security]: struct.NamedTempFile.html#security
    /// [resource-leaking]: struct.NamedTempFile.html#resource-leaking
    pub fn make<F, R, J: Fn(&OsStr, &OsStr) -> PathBuf>(&self, f: F, join: J) -> io::Result<NamedTempFile<R>>
    where
        F: FnMut(&Path) -> io::Result<R>,
    {
        self.make_in(env::temp_dir(), f, join)
    }
    ...
    /// # Ok::<(), std::io::Error>(())
    /// ```
    pub fn make_in<F, R, P, J: Fn(&OsStr, &OsStr) -> PathBuf>(&self, dir: P, mut f: F, join: J) -> io::Result<NamedTempFile<R>>
    where
        F: FnMut(&Path) -> io::Result<R>,
        P: AsRef<Path>,
    {
